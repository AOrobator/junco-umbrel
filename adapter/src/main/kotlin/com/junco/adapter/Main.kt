...
fun Application.juncoModule(
    config: AppConfig = loadConfig(),
    electrum: ElectrumGateway = ElectrumClient()
) {
    configureEnvironment(config)

    val jsonConfig = Json { ignoreUnknownKeys = true; encodeDefaults = true }
    val authStore = AuthStore(config.dataDir.resolve("auth.json"), jsonConfig)
...
fun loadConfig(): AppConfig {
    val dataDir = Path.of(
        System.getProperty("junco.data")
            ?: System.getenv("JUNCO_DATA")
            ?: "/data"
    )
    val network = parseNetwork(
        System.getProperty("junco.network")
            ?: System.getenv("JUNCO_NETWORK")
            ?: "mainnet"
    )
    return AppConfig(dataDir, network)
}

fun loadPort(): Int {
    return System.getProperty("junco.port")?.toIntOrNull()
        ?: System.getenv("JUNCO_PORT")?.toIntOrNull()
        ?: 8081
}

fun configureEnvironment(config: AppConfig) {
